...
class sensorMarkVidFirst() {
    fun drawCotLocal(
        ...
        sensor.setAttribute("videoUID", "hello")
        sensor.setAttribute("videoUrl", "http://255.251.123.1")
        //sensor.setAttribute(HIDE_FOV, false.toString())

        link.setAttribute("uid", "VideoTest");
        link.setAttribute("type", "b-x-u");
        link.setAttribute("relation", "p-p");
        link.setAttribute("mime", "video/aaaaa");
        link.setAttribute("url", "http://255.251.123.1");

        sensor.addChild(link)
